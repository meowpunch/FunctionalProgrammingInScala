...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum(tail) of a few numbers (10pts)") {
    assert(sumWithAcc(List(1,2,0), 0) == 3)
  }

  test("max(tail) of a few numbers (10pts)") {
    assert(maxWithLocalMax(List(3, 7, 2), Int.MinValue) == 7)
  }
